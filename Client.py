...
def dhcp_request(xid, mac_address, ip_address):
    ...
    message = packet
    client.sendto(packet.asbytes, ("localhost", 4421))
...
def timeout_handler(prev_interval):
    ...
    while live:
        ...
        if not ip_discovered:
            ...
            global alive
            alive = True
            client.sendto(b'help', ("localhost", 4422))
        else:
            interval = prev_interval
            live = False
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while alive:
            data, addr = client.recvfrom(1024)
            try:
                regular_string = data.decode('utf-8')
            except UnicodeDecodeError as e:
                print(f"Error decoding byte string: {e}")
                regular_string = ''
            if regular_string == 'help':
                break
            dhcp_packet = dhcppython.packet.DHCPPacket.from_bytes(data)
            message_type = dhcp_packet.msg_type
